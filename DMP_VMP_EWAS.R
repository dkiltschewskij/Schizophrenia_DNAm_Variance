...
# set predicted sex and Sentrix_ID to factor
pheno$predicted_sex_bin<-factor(pheno$predicted_sex_bin,levels=c(0,1))
...
testCpG<-function(row, phe){
  model<-lm(row ~phe$Status_Kiltschewskij+phe$horvath.age+phe$predicted_sex_bin+phe$smokingScore+phe$Sentrix_ID+phe$Sentrix_Position+phe$CD4T+phe$CD8T+phe$NK+phe$Mono+phe$Gran+phe$Bcell,na.action = na.exclude)
...
varCpG<-function(row, phe){
  ...
  resid<-residuals(lm(row ~phe$horvath.age+phe$predicted_sex_bin+phe$smokingScore+phe$Sentrix_ID+phe$Sentrix_Position+phe$CD4T+phe$CD8T+phe$NK+phe$Mono+phe$Gran+phe$Bcell,na.action = na.exclude))
...
for (n in seq(0,850000,by=25000)){
  ...
  # Save results
  write.table(res, file=paste("EWAS_",opt$out,"_chunk_",skip,".txt",sep=""),quote=F,sep="\t",row.names = F)
